...
func Test_b64toMinIOStringURL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			test: "valid encoded url but not coming from FST server returns forbidden error", // http://non-minio-host:9000/...
			args: args{
		...
		{
			test: "valid encoded url but not coming from FST server port returns forbidden error", // other port http://localhost:8902/...
			args: args{
